...
function setBonusGameAccess() {
  ...
  state.bonusUnlocked = pct >= 60;

  if (state.bonusUnlocked) {
    if (!state.bonusGameType) {
      state.bonusGameType = Math.random() > 0.5 ? 'breakout' : 'pong';
    }
    miniGameBtn.classList.remove('hidden');
    miniGameBtn.disabled = false;
    miniGameBtn.textContent = state.bonusGameType === 'breakout' ? 'Play 1-Minute Breakout' : 'Play 1-Minute Pong';
...
function startMiniGame() {
  ...
  const gameType = state.bonusGameType || 'breakout';
  resultScreen.classList.add('hidden');
...
function finishMiniGame() {
  ...
  gameInstructions.textContent = `Time is up! Mini-game score: ${gameState.score}.`;
